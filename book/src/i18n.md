...
sequenceDiagram
    ...
    participant Backend
    participant Config as config/i18n/

    Browser->>Frontend: 加载页面 (Accept-Language: en)
    Frontend->>Backend: GET /api/locales
    Backend->>Config: 扫描 config/i18n/*.json
    Backend-->>Frontend: [{code: "en", name: "English"}, ...]
    Frontend->>Frontend: resolveInitialLocale() → "en"
    Frontend->>Backend: GET /api/locales/en
    Backend->>Config: 读取 en.json
    Backend-->>Frontend: {"site.title": "...", ...}
    Frontend->>Frontend: t("site.title") → "25 ECE 3 Todo List"
```
